#ifndef TEAM02_EDGE_ITER_V4_H
#define TEAM02_EDGE_ITER_V4_H
...
namespace ei4 {
    ...
    void edge_iterator(TRL* lister,AdjacencyGraph<Index> *G, void *dummy = nullptr) {
        ...
        for (Counter si = 0; si < Gn; si++) {  // this should not count toward op count
            ...
            s_adj = &G_adj[(index_t) s];
            s_adj_neighbors = s_adj->neighbors;
            s_adj_count = s_adj->count;
            // iterate over all edges (neighbors) of node s
            for (Counter ti = 0; ti < s_adj_count; ti++) {
                t = s_adj_neighbors[ti];
                t_adj = &G_adj[(index_t) t];
