#ifndef TEAM02_FORWARD_HASHED_V3_H
#define TEAM02_FORWARD_HASHED_V3_H

#include "common.h"
#include "adjacency_graph.h"
#include "triangle_lister.h"
#include "hash_table_v3.hpp"
        ...
        for (Counter ti = 0; ti < G->adjacency[(index_t) s].count; ti++) {
            ...
            if (s < t) {
                ...
                for (Counter i = 0; i < HASH_CONTAINER_SIZE; i++) {
                    HashItem<Index> *head = &probe->head[i];
                    do {
                        for (Counter j = 0; j < HASH_ITEM_SIZE; j++) {
                            if (head->number[j] != HASH_NULL_NUMBER) {
                                if (hashtable_lookup(build, head->number[j])) {
