#ifndef TEAM02_FORWARD_HASHED_STD_H
#define TEAM02_FORWARD_HASHED_STD_H

#include "common.h"
#include "adjacency_graph.h"
#include "triangle_lister.h"
#include "hash_table_std.hpp"
    ...
    for (Counter si = 0; si < G->n; si++) {  // this should not count toward op count
        ...
        for (Counter ti = 0; ti < G->adjacency[(index_t) s].count; ti++) {
            ...
            if (s < t) {
                ...
                for (const auto &number: *probe) {
                    if (hashtable_lookup(build, number)) {
                        lister->list_triangle(s, t, number);
                    }
                }
