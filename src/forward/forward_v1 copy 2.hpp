...
void forward(TRL* lister,AdjacencyGraph<Index> *G, ForwardNeighborContainer<Index> *A) {
    ...
    for (Counter si = 0; si < G->n; si++) {  // this should not count toward op count
        ...
        for (; ti + 3 < G->adjacency[(index_t) s].count; ti+=4) {
            ...
            __m256i at_count = _mm256_set_epi64x(At_3->count, At_2->count, At_1->count, At_0->count);
            ...
            do {
                ...
                __m256i s_neigbours = _mm256_set_epi64x(As->neighbors[i_mat[3]], As->neighbors[i_mat[2]], As->neighbors[i_mat[1]], As->neighbors[i_mat[0]]);
                __m256i t_neigbours = _mm256_set_epi64x(At_3->neighbors[j_mat[3]], At_2->neighbors[j_mat[2]], At_1->neighbors[j_mat[1]], At_0->neighbors[j_mat[0]]);
