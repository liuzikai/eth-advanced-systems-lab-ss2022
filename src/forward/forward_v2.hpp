#ifndef TEAM02_FORWARD_V2_H
#define TEAM02_FORWARD_V2_H
    ...
    for (Counter si = 0; si < G->n; si++) {  // this should not count toward op count
        ...
        for (; ti + 3 < G->adjacency[(index_t) s].count; ti+=4) {
            ...
            do {
                ...
                __m256i s_neigbours = _mm256_set_epi64x(
                    (index_t)As->neighbors[i_mat[3]], (index_t)As->neighbors[i_mat[2]], 
                    (index_t)As->neighbors[i_mat[1]], (index_t)As->neighbors[i_mat[0]]);
                __m256i t_neigbours = _mm256_set_epi64x(
                ...
                if constexpr (std::is_same_v<TRL, TriangleListing::Count<Index>>) {
                    ...
                    if(trianlge_mask) {
                        ...
                        uint64_t s_neigbours_mat[8];
                        _mm256_store_si256((__m256i *)s_neigbours_mat, s_neigbours);
                        for (uint32_t k = 0, index = 0; k < 32; k += 8, index++) {
