...
AdjacencyGraph<Index> *create_graph_from_file(const char *filename) {
    ...
    if (!f) throw std::runtime_error("Invalid input: fail to open the file");
    ...
    if (res != 1) throw std::runtime_error("Invalid input: missing n");
    G->n = read_val;
    ...
    for (index_t u = 0; u < G->n; u++) {
        ...
        res = fscanf(f, INDEX_FMT, &read_val);
        if (res != 1) throw std::runtime_error("Invalid input: missing node count");
        adj->count = read_val;
