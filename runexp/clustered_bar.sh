...
python $RUNEXP/clustered_bar.py -m $METADATA -d $DATADIR -p $DATADIR -c $COMMIT -f $WARMUP"-"$RUN"-"$PHASE -n $NUMBER -g "US_PATENTS,GERMAN_ROAD_NETWORK,GOOGLE_CONTEST,WWW_NOTRE_DAME"

# python $RUNEXP/clustered_bar.py -m $METADATA -d $DATADIR -p $DATADIR -c $COMMIT -f $WARMUP"-"$RUN"-"$PHASE -n $NUMBER -g "GERMAN_ROAD_NETWORK,COMP_SCIENCE_AUTHORS,GOOGLE_CONTEST,HELP_LITERATURE,WWW_NOTRE_DAME,US_PATENTS"
