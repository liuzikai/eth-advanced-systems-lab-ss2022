...
if [ $GRAPHTYPE = "GENERATED" ]; then
    EXPDIR=$DATADIR/"high-"$ALGO"-"$COMMIT"-"$GRAPHTYPE"-"$LOWNODE"-"$HIGHNODE"-"$INTERVAL"-"$DEGREE
    if [ ! -z $SEED ]; then
        EXPDIR=$EXPDIR"-"$SEED
    fi
else
...
if [ $GRAPHTYPE = "GENERATED" ]; then
    ...
    do
        ...
        EDGE=$(($i * $AVG_D / 2))
        if [ -z $SEED ]; then
            graph=$PREFIX$i"_"$EDGE
        else
            graph=$PREFIX$SEED"_"$i"_"$EDGE
        fi
        echo "+ $graph"
        if [ -z $SEED ]; then
            # always generate random graph without seed
            python3 $RUNEXP/generate_graph.py --gt $GRAPHTYPE --num_nodes $i --num_edges $EDGE --shuffle_edges --density EXIST -o $INPUTDIR/$graph.txt
            ...
            if [ ! -f $INPUTDIR/$graph.txt ]; then
                python3 $RUNEXP/generate_graph.py --gt $GRAPHTYPE --num_nodes $i --num_edges $EDGE --seed $SEED --shuffle_edges --density EXIST -o $INPUTDIR/$graph.txt
    ...
    if [ -z $SEED ]; then
        python3 plot_speedup.py -d $EXPNUM -p $EXPNUM -n $DEGREE -l $LOWNODE -r $HIGHNODE -i $INTERVAL -t -b $BASE -f $PREFIX
    else
        python3 plot_speedup.py -d $EXPNUM -p $EXPNUM -n $DEGREE -l $LOWNODE -r $HIGHNODE -i $INTERVAL -s $SEED -t -b $BASE -f $PREFIX
    fi
else
    graph=$GRAPHTYPE
    echo "+ $graph"
