...
TEST_CASE("forward_hashed: example graph") {
    ...
    for (int i = 0; i < 3; i++) {  // repeat
        TriangleListing::SetCollect<index_t> lister;
        forward_hashed<index_t, index_t, TriangleListing::SetCollect<index_t>>(&lister, G, A);
    ...
    REQUIRE(lister.triangles == TriangleListing::SetCollect<index_t>::TriangleSet{{0, 1, 2}, {0, 1, 3}, {0, 3, 4}, {0, 5, 6}, {0, 7, 8}});
    free_graph(G);
    forward_hashed_delete_neighbor_container(A);
}
...
TEST_CASE("forward_hashed: WWW_NOTRE_DAME graph") {
    ...
    forward_hashed<index_t, index_t, TriangleListing::SetCollect<index_t>>(&triangle_listing, graph, A);
