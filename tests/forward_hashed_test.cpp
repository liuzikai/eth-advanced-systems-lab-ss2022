...
TEST_CASE("forward_hashed: example graph") {
    ...
    for (int i = 0; i < 3; i++) {  // repeat
        auto lister = forward_hashed<index_t, index_t, TriangleListing::SetCollect<index_t>>(G, A);
...
TEST_CASE("forward_hashed: WWW_NOTRE_DAME graph") {
    ...
    auto triangle_listing = forward_hashed<index_t, index_t, TriangleListing::SetCollect<index_t>>(graph, A);
